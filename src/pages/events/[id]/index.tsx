...
export default function About({ id }: InferGetServerSidePropsType<typeof getServerSideProps>) {
    ...
    const [responseData, setResponseData] = useState(testGridData);

    if (!id) {
        return (
            <Flex direction='column'>
                <Flex justify='center' direction='row' w='100%' h='140px' px={[2, 4, 5, 5]} bg='#F4F7F9' borderBottom='1px dashed #dcdee0'>
                    <Flex align='center' direction='row' w={['100%', '100%', '70%', '70%']} minW='200px' maxW='920px' h='100%' px={[0, 1, 2, 4]} borderRight='1px dashed #dcdee0' borderLeft='1px dashed #dcdee0'>
                        <Text mt='50px' mr={6} ml={2} color='#625BF8' fontSize='38px' fontWeight={600}>
                            Event does not exist :(                        </Text>
                    </Flex>
                </Flex>
            </Flex>
        );
                ...
                <Flex direction='column' w='100%' minW='200px' maxW='800px' h='100%' px={[0, 1, 2, 4]} py={4} borderRight='1px dashed #dcdee0' borderLeft='1px dashed #dcdee0'>
                    <Flex align={['center', 'start', 'start', 'start']} direction={['column', 'column', 'row', 'row']} w='100%'>
                        <Flex direction='column' w='40%' minW='320px' ml={2}>
                            <Flex>
                                <Flex align='center' justify='center' maxW='170px' h='52px' mt={4} mr={1} px={3} bg='#E6E7F9' borderRadius={5} shadow='0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);' _hover={{transform: 'translateY(-1px)', bg: '#ebecfc', cursor: 'pointer', boxShadow: '0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);'}} onClick={onRespond}>
                                    <HStack>
                                        ...
                                        <Text color='#625BF8' fontSize='16pt' fontWeight={600}>{addingPerson ? 'Cancel' : 'Respond'}</Text>
                                
                                    </HStack>
                                </Flex>
                                <Flex align='center' justify='center' w='170px' maxW='130px' h='52px' mt={4} mr={1} ml={4} bg='#FFF' border='2px solid #F4F7F9' borderRadius={5} shadow='0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);' _hover={{transform: 'translateY(-1px)', bg: '#F9f9f9', cursor: 'pointer', boxShadow: '0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);'}} onClick={onCopyLink}>
                                    <HStack>
                                        {inviteText === 'Invite' && <BsLink color='#625BF8' fontSize='18pt' />}
                                        {inviteText === 'Copied' && <BsCheckLg color='#625BF8' fontSize='18pt' />}
                                        <Text color='#625BF8' fontSize='16pt' fontWeight={600}>{inviteText}</Text>
                            ...
                            <HStack mt={5}  ml={5}>
                                ...
                                <Text  color='#818181' fontSize='15pt' fontWeight={600}>Responses</Text>
                            </HStack>
                            <Flex direction='column' mt={3} ml='22px' >
                                <Flex align='center' mb={3.5}>
                                    <Input maxW='165px' h='32px' px={2} color='#00142C' fontSize='13pt' fontWeight={600} bg='#F4F7F9' border='2px solid lightgray' borderRadius={2} _hover={{border: '2px solid #7A84E8'}} _focus={{boxShadow: 'none', outline: 'none', border: '2px solid #7A84E8'}} _placeholder={{color: 'gray'}} placeholder='Name' spellCheck='false' />
                                ...
                                <Flex align='center' mt={1.5}>
                                    ...
                                    <Text ml={2.5} color='#00132C' fontSize='15pt' fontWeight={600}>Matt</Text>
                                </Flex>
                                <Flex align='center' mt={1.5}>
                                    <Checkbox w='20px' h='20px' p={0} borderColor='#D3D3D3' borderRadius={3} colorScheme='twitter'  size='lg' />
                                    <Text ml={2.5} color='#00132C' fontSize='15pt' fontWeight={600}>Matt</Text>
                        ...
                        <Flex direction='column' w='60%' minW='350px' ml={2}>
                            <Flex align='center' px={2}>
                                <Text mt={4} color='#00142C' fontWeight={600}>{`0/${responseData.length}`}</Text>
                                <Flex align='center' justify='center' w='270px' h='52px' mt={4} mr={1} ml={2} px={3} bg='#E6E7F9' borderRadius={5} shadow='0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);' _hover={{transform: 'translateY(-1px)', bg: '#ebecfc', cursor: 'pointer', boxShadow: '0 7px 14px rgba(50, 50, 93, 0.08), 0 3px 2px rgba(0, 0, 0, 0.06);'}}>
                                    <HStack h='100%' spacing={0}>
                                        {responseData.map((r, i) => {
                                            return (
                                                <Flex key={r.name} align='center' justify='center' w={`${270 / responseData.length}px`} h='100%' bg='#8983FC' opacity={i / (responseData.length - 1)} borderRightRadius={i === responseData.length - 1 ? 5 : 0} />
                                            );
                                        })}
                                    </HStack>
                                </Flex>
                                <Text mt={4} ml={1} color='#00142C' fontWeight={600}>{`${responseData.length}/${responseData.length}`}</Text>
